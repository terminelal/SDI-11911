...
double* getMotorValue(int x_velocity, int y_velocity, int w_velocity){
    ...
    velMots[0] = .1;//(double)( s1 * velXMod + c1 * velYMod + velWMod);
    velMots[1] = .1;//(double)( s1 * velXMod + c1 * velYMod + velWMod);
	velMots[2] = (double)(w_velocity);
...
int main(int argc, char **argv){
	...
	effort [3] = velMots[3];
		...
		if(client.exists()){
			...
			// Wheel-Joint 3
			eff_msg[2].request.joint_name = "steer_join";
