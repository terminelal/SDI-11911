...
void getRobotPose(const geometry_msgs::Pose2D& msg) {
    // msg.name[1]
    rposition.position.x = msg.x;
    rposition.position.y = msg.y;
...
int main(int argc, char **argv){
	...
	ros::Publisher pub_vel_turtle = nh.advertise<geometry_msgs::Pose2D>("/target_vel_topic", rate_hz);
	...
	while (ros::ok())
	{
		...
		if(distancia > 0){
			...
			desired_velocity.x = distancia;
			desired_velocity.y = 0; // dist
			desired_velocity.theta = volante;
		} else {
