...
//Asignamos un rate en Hz para la ejecución del programa
double rate_hz = 5; 

//Función empleada para conocer la posición del robot respecto al 
//mundo (Gazebo).
...
//Función que nos permite conocer la pose a la que el robot tiene
//que llegar.
...
//Función que genera una velocidad constante entre la pose actual
//y la pose deseada.
geometry_msgs::Twist generateConstantVelocity(double constant_speed, geometry_msgs::Twist p_start, geometry_msgs::Twist p_goal){
    ...
    //Declaración y especificación del modelo cinemático del robot.
	double alfa = atan2(p_goal.linear.y, p_goal.linear.x);
	double steer = alfa; 
	double norma;

	//Limitaciónes para el steering. ¡¡¡SI NO, SE VUELVE LOCO!!!
	if(steer > 0.5)
		steer = 0.5;
	if(steer < -0.5)
		steer = -0.5;
...
//Función limitante de la velocidad para que no se vuelva loco.
...
int main(int argc, char **argv){
	...
	//Definimos la publicación en el tópico correspondiente, en este caso vel_turtle.
	ros::Publisher pub_vel_turtle = nh.advertise<geometry_msgs::Twist>("/target_vel_topic", rate_hz);

	//Subscripción a los tópicos necesarios para el funcionamiento del programa.
	...
	//Publicamos la velocidad calculada en el tópico definido más arriba.
